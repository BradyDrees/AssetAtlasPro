...
export function ProSidebar({ user, hasPmRole = false, hasOwnerRole = false, vendorRole = "owner" }: ProSidebarProps) {
  ...
  const navItems = isTech
    ? [
        { href: "/pro/schedule", label: vt("mySchedule"), icon: <CalendarIcon />, matchExact: false },
        { href: "/pro/jobs", label: vt("myJobs"), icon: <BriefcaseIcon />, matchExact: false },
    ...
    : isOfficeMgr
      ...
      : [
          ...
          { href: "/pro/pricebook", label: vt("pricebook"), icon: <PricebookIcon />, matchExact: false },
          { href: "/pro/templates", label: vt("templates"), icon: <TemplatesIcon />, matchExact: false },
          { href: "/pro/invoices", label: vt("invoices"), icon: <ReceiptIcon />, matchExact: false },
          { href: "/pro/recurring", label: vt("recurring"), icon: <ReceiptIcon />, matchExact: false },
          ...
          { href: "/pro/reviews", label: vt("reviews"), icon: <StarIcon />, matchExact: false },
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div
          className="p-4 flex items-center justify-between bg-[radial-gradient(ellipse_at_top_left,_var(--color-amber-900)_0%,_transparent_50%),radial-gradient(ellipse_at_bottom_right,_var(--color-amber-900)_0%,_var(--charcoal-950)_60%)] bg-charcoal-900"
          style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 16px)" }}
        >
          {!collapsed && (
            <Link href="/pro" className="flex-shrink-0">
              <div>
                <h1 className="text-xl font-bold text-white leading-tight">Atlas <span className="text-amber-400">Pro</span></h1>
                <p className="text-sm text-gold-400">{vt("proPortal")}</p>
              </div>
            </Link>
          )}
