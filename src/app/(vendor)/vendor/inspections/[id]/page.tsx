...
export default async function InspectionDetailPage({
  ...
  return (
    <div>
      ...
      <div className="bg-[radial-gradient(ellipse_at_top_left,_var(--color-amber-900)_0%,_transparent_50%),radial-gradient(ellipse_at_bottom_right,_var(--color-amber-900)_0%,_var(--charcoal-950)_60%)] bg-charcoal-900 -mx-4 -mt-4 md:-mx-6 md:-mt-6 px-4 py-5 md:px-6 rounded-b-xl mb-6">
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 text-sm text-amber-300 mb-3">
          <Link
            href="/vendor/inspections"
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-amber-200 mt-1">{project.property_name}</p>
              {project.address && (
                <p className="text-sm text-amber-300/70 mt-0.5">{project.address}</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link
              href={`/vendor/inspections/${id}/review`}
            ...
            <InspectionHeaderMenu project={project} role={role} tier="vendor" />
          </div>
        </div>
      </div>
