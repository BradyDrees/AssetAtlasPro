...
import { createDealAnalysis, updateDealAnalysis } from "@/app/actions/deal-analysis";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        ...
        const dealId = await createDealAnalysis({
          name,
          property_name: propertyName,
          address,
        });
        onClose();
        router.push(`/deal-analysis/${dealId}`);
      }
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-edge-secondary rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 uppercase bg-surface-primary text-content-primary"
        />
      ...
      <div>
        <label htmlFor="propertyName" className="block text-sm font-medium text-content-secondary mb-1">
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-edge-secondary rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 bg-surface-primary text-content-primary"
        />
      </div>
      <div>
        <label htmlFor="address" className="block text-sm font-medium text-content-secondary mb-1">
          {t("forms.address")}
        </label>
        <input
          id="address"
