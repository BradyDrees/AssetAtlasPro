...
interface PmInviteModalProps {
  open: boolean;
  onClose: () => void;
}

export function PmInviteModal({ open, onClose }: PmInviteModalProps) {
  ...
  const handleInvite = useCallback(async () => {
    ...
    const res = await inviteVendor(email.trim());
    if (res.success && res.inviteLink) {
      setResult({
        type: "success",
        text: t("pmVendors.inviteSent"),
        link: res.inviteLink,
      });
      setEmail("");
      ...
      setResult({
        type: "error",
        text: res.error ?? t("pmVendors.inviteError"),
      });
    }
    setSending(false);
  }, [email, t, router]);
